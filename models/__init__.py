...
def init_schema():
    ...
    # Migration check for existing SQLite databases created before qr_token was added
    cols = [r["name"] for r in db.execute("PRAGMA table_info(patients)").fetchall()]
    if "qr_token" not in cols:
        db.execute("ALTER TABLE patients ADD COLUMN qr_token TEXT")
    
    # Populate missing QR tokens for existing patients
    unseeded = db.execute("SELECT id FROM patients WHERE qr_token IS NULL OR qr_token = ''").fetchall()
    for row in unseeded:
        token = secrets.token_urlsafe(24)
        db.execute("UPDATE patients SET qr_token = ? WHERE id = ?", (token, row["id"]))
