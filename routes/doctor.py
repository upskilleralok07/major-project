...
from flask import Blueprint, render_template, request, g, send_file, flash, redirect, url_for

from models import report as report_model, patient as patient_model
...
# ------------------------------------------------------------- Patient QR Code Access

@bp.route("/qr-access/<qr_token>")
def qr_access(qr_token):
    if not g.get("user") or g.user["role"] != "doctor":
        flash("Please log in as a doctor to view patient medical reports via QR code scan.", "info")
        return redirect(url_for("auth.login", next=request.path))

    pat = patient_model.get_by_qr_token(qr_token)
    if not pat:
        raise AppError(404, "Invalid or revoked QR code token. The patient may have regenerated their QR code.")

    q = request.args.get("q", "").strip()
    category = request.args.get("category", "")
    reports_list = report_model.search(pat["id"], q=q, category=category, sort="newest")
    categories = report_model.categories()

    log_activity("QR Code scanned", f"Doctor {g.user['name']} accessed profile & reports of patient {pat['name']}",
                 target_user_id=pat["user_id"])

    return render_template("doctor/patient_qr_portal.html", pat=pat, reports=reports_list,
                           categories=categories, qr_token=qr_token, q=q, category=category)


@bp.route("/qr-access/<qr_token>/reports/<int:report_id>/preview")
@doctor_only
def qr_report_preview(qr_token, report_id):
    pat = patient_model.get_by_qr_token(qr_token)
    if not pat:
        raise AppError(404, "Invalid or revoked QR code token.")
    rep = report_model.get(report_id)
    if not rep or rep["patient_id"] != pat["id"]:
        raise AppError(403, "Report does not belong to this patient.")

    path = file_handler.file_path(pat["id"], rep["stored_name"])
    if not os.path.exists(path):
        raise AppError(404, "The file for this report could not be found on the server.")
    return send_file(path, mimetype=file_handler.SERVE_MIME[rep["file_type"]], as_attachment=False,
                     download_name=rep["file_name"])


@bp.route("/qr-access/<qr_token>/reports/<int:report_id>/download")
@doctor_only
def qr_report_download(qr_token, report_id):
    pat = patient_model.get_by_qr_token(qr_token)
    if not pat:
        raise AppError(404, "Invalid or revoked QR code token.")
    rep = report_model.get(report_id)
    ...
    if not os.path.exists(path):
        raise AppError(404, "The file for this report could not be found on the server.")
    log_activity("Report downloaded", f"Downloaded '{rep['title']}' of patient {pat['name']} via QR access",
                 report_id=report_id, target_user_id=pat["user_id"])
    return send_file(path, mimetype=file_handler.SERVE_MIME[rep["file_type"]], as_attachment=True,
                     download_name=rep["file_name"])
