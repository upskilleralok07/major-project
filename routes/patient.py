...
from flask import (Blueprint, render_template, request, redirect, url_for, flash, g, jsonify, send_file, Response)
...
from utils.qr_generator import generate_qr_svg
...
@bp.route("/profile/qr.svg")
@login_required
def profile_qr_svg():
    pat = patient_model.get_by_user(g.user["id"]) if g.user["role"] == "patient" else None
    if not pat:
        raise AppError(404, "Patient profile not found.")
    token = pat["qr_token"] if pat["qr_token"] else patient_model.ensure_qr_token(pat["id"])
    target_url = url_for("doctor.qr_access", qr_token=token, _external=True)
    svg_data = generate_qr_svg(target_url)
    return Response(svg_data, mimetype="image/svg+xml")


@bp.route("/profile/qr/reset", methods=["POST"])
@patient_only
def reset_qr_token():
    patient_model.reset_qr_token(g.patient["id"])
    log_activity("QR Code regenerated", "Regenerated patient access QR code token")
    flash("Your Medical Access QR Code has been regenerated. Previous QR codes are now invalid.", "success")
    return redirect(url_for("patient.profile"))
